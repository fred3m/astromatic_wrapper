...
class TestAstromatic:
    ...
    def test_build_cmd(self,tmpdir):
        ...
        kwargs = copy.deepcopy(sex_kwargs)
        sextractor = api.Astromatic(**kwargs)
        cmd_result = 'sex test.fits -c {0} -FILTER N -PARAMETERS_NAME '.format(
            sex_kwargs['config_file'])
        cmd_result += 'default.path -CATALOG_TYPE FITS_LDAC -CATALOG_NAME {0}'.format(
        ...
        cmd_result = 'path/to/sex test.fits -FILTER N -PARAMETERS_NAME {0} '.format(
            os.path.join(str(tmpdir), 'sex.param'))
        cmd_result += '-CATALOG_TYPE FITS_LDAC -CATALOG_NAME {0}'.format(
            sex_kwargs['config']['CATALOG_NAME'])
...
def test_run_sex(tmpdir):
    ...
    cmd = ('sex img.fits -PARAMETERS_NAME default.path -WRITE_XML Y -XML_NAME '
        '{0}/0.sex.log.xml -CATALOG_NAME img.cat '.format(paths['log']))
    ...
    cmd = 'sex img.fits[1] -PARAMETERS_NAME default.path -WRITE_XML Y -XML_NAME '
    cmd += '{0}/0.sex.log-1.xml -CATALOG_NAME img.cat '.format(paths['log'])
...
def test_run_scamp(tmpdir):
    ...
    setattr(__builtin__,'raw_input', mock_raw_input('y'))
    pipe = pipeline.Pipeline(paths=paths, build_paths = {})
    result = api.run_scamp(pipe,0,['cat1.fits','cat2.fits'], {}, 'new_cat')
    cmd = 'scamp cat1.fits cat2.fits -SAVE_REFCATALOG Y -WRITE_XML Y -XML_NAME '
    cmd += '{0}/0.scamp.log.xml -REFOUT_CATPATH new_cat'.format(paths['log'])
    cmd_result = {
        'args': (
            cmd,
            False,
...
def test_run_swarp(tmpdir):
    ...
    cmd = 'swarp img1.fits img2.fits -WRITE_XML Y -XML_NAME '
    cmd += '{0}/0.swarp.log.xml '.format(paths['log'])
    cmd += '-RESAMPLE_DIR {0}'.format(paths['temp'])
    cmd_result = {
        'args': (
            cmd,
            False,
    ...
    cmd = 'swarp img1.fits[1] img2.fits[1] -WRITE_XML Y -XML_NAME '
    cmd += '{0}/0.swarp.log-1.xml '.format(paths['log'])
    cmd += '-RESAMPLE_DIR {0}'.format(paths['temp'])
    cmd_result = {
        'args': (
            cmd,
    ...
    assert result['kwargs']==cmd_result['kwargs']
